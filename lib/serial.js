...
function postHex(reqCode, serverIp, serverPort) {
	try {
		...
		console.log("连接服务器中...");
		// var socket = new Socket("172.16.34.132", 5001);
		// var socket = new Socket(serverIp, serverPort);
		// var socket  = new Socket("60.166.53.42", 8050);
		var socket = getApp().globalData.socket;
		console.log("mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm")
		if (getApp().globalData.socket == null) {
			console.log("this will connect socket");
			getApp().globalData.socket = new Socket("60.166.53.42", 8050);
		}
		...
		if (ret[2] == 4) { //实时流量
			...
			actualFlow = ret[9] + "" + ret[10] + "" + ret[7] + "" + ret[8] + "" + ret[5] + "" + ret[6] + "" + ret[3] +
				"" + ret[4];
		}
